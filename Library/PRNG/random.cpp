...
Random :: Random(const std::string path){
  // Constructor that reads the seed from a file
  int seed[4];
  int p1, p2;
  ifstream Primes(path + "Primes");
  if (Primes.is_open()){
    Primes >> p1 >> p2 ; //cambiando questi genero una nuova sequenza statisticamente scorrelata
...
double Random :: CauchyLorentz(double mu, double gamma){
    // This function generates a random number from a Cauchy-Lorentz distribution with given x0 and gamma
    double y = Rannyu();
...
double Random :: DiscreteMovement(){
  return floor(Rannyu(0,6));
}
