...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    for (int i=0; i<_npart; i++){
      ...
      double num = _rnd.Rannyu();
      double v_value_1 =pow(2,1.5)*sqrt(_temp)/sqrt(M_PI);
      //Delta distribution of the velocity modulus
      if (num > (1. / 3.)) {
        if (num > (2. / 3.)) {
            vy(i) = _rnd.randomSign() * v_value_1;
        } else {
            vz(i) = _rnd.randomSign() * v_value_1;
        }
    } else {
        vx(i) = _rnd.randomSign() * v_value_1;
    }
  ...
  // Scrivi le velocità iniziali
  this->write_initial_velocities();
  return;
}
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        _bin_size_v = v_max/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4
        _measure_pofv = true;
...
void System :: write_last_block(int block)
{
  if (_sim_type > 1 and _nblocks == block and _restart){
    if (_measure_chi and _H == 0.0 and _restart){
      ofstream coutChi;
      if (_sim_type == 2) coutChi.open("../OUTPUT/METROPOLIS/chi.dat", ios::app);
      if (_sim_type == 3) coutChi.open("../OUTPUT/GIBBS/chi.dat", ios::app);
      if (coutChi.is_open()){
        double errorChi = error(_global_av(_index_chi),_global_av2(_index_chi), this->get_nbl());
        coutChi << setw(12) << _temp
						    << setw(12) << _global_av(_index_chi)/double(block)
						    << setw(12) << errorChi << endl;
      } else cerr << "PROBLEM: Unable to open chi.dat" << endl;
      coutChi.close();
    }
    if (_measure_cv and _H == 0.0 and _restart){
      ofstream coutCv;
      if (_sim_type == 2) coutCv.open("../OUTPUT/METROPOLIS/cv.dat", ios::app);
      if (_sim_type == 3) coutCv.open("../OUTPUT/GIBBS/cv.dat", ios::app);
      if (coutCv.is_open()){
        double errorCv = error(_global_av(_index_cv),_global_av2(_index_cv), this->get_nbl());
        coutCv << setw(12) << _temp
                  << setw(12) << _global_av(_index_cv)/double(block)
                  << setw(12) << errorCv << endl;
      } else cerr << "PROBLEM: Unable to open cv.dat" << endl;
      coutCv.close();
    }
    
    if (_measure_magnet and _H != 0.0 and _restart){
      ofstream coutMag;
      if (_sim_type == 2) coutMag.open("../OUTPUT/METROPOLIS/magnet.dat", ios::app);
      if (_sim_type == 3) coutMag.open("../OUTPUT/GIBBS/magnet.dat", ios::app);
      if (coutMag.is_open()){
        double errorMag = error(_global_av(_index_magnet),_global_av2(_index_magnet), this->get_nbl());
        coutMag << setw(12) << _temp
                  << setw(12) << _global_av(_index_magnet)/double(block)
                  << setw(12) << errorMag << endl;
      } else cerr << "PROBLEM: Unable to open magnet.dat" << endl;
      coutMag.close();
    }
    if (_measure_tenergy and _restart){
      ofstream coutTenergy;
      if (_sim_type == 2) coutTenergy.open("../OUTPUT/METROPOLIS/tenergy.dat", ios::app);
      if (_sim_type == 3) coutTenergy.open("../OUTPUT/GIBBS/tenergy.dat", ios::app);
      if (coutTenergy.is_open()){
        double errorTenergy = error(_global_av(_index_tenergy),_global_av2(_index_tenergy), this->get_nbl());
        coutTenergy << setw(12) << _temp
                  << setw(12) << _global_av(_index_tenergy)/double(block)
                  << setw(12) << errorTenergy << endl;
      } else cerr << "PROBLEM: Unable to open tenergy.dat" << endl;
      coutTenergy.close();
    }
...
void System :: write_configuration(){
  ...
  if(_sim_type < 2){
    ...
    coutf.open("../OUTPUT/CONFIG/config.ising");
...
void System::write_initial_velocities() {
  ofstream coutv("../OUTPUT/initial_velocities.dat");
  if (coutv.is_open()) {
      coutv << "# Particle velocities at the initial measurement\n";
      coutv << "# Index    vx          vy          vz\n";
      for (int i = 0; i < _npart; i++) {
          coutv << setw(8) << i
                << setw(12) << _particle(i).getvelocity(0)
                << setw(12) << _particle(i).getvelocity(1)
                << setw(12) << _particle(i).getvelocity(2) << endl;
      }
      coutv.close();
      cout << "Initial velocities written to ../OUTPUT/initial_velocities.dat" << endl;
  } else {
      cerr << "PROBLEM: Unable to open ../OUTPUT/initial_velocities.dat" << endl;
  }
}
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_tenergy and _measure_cv){
    _measurement(_index_cv) = pow(_measurement(_index_tenergy) * double(_npart), 2);
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
...
void System :: averages(int blk){
  ...
  if (_measure_cv and _measure_tenergy){
    ...
    _block_av(_index_cv) *= pow(_beta, 2)/double(_npart);
    }
		//_average(_index_cv) = (pow(_beta, 2) * (_block_av(_index_cv)/double(_nsteps) - pow(_block_av(_index_tenergy)*double(_npart), 2)/double(_nsteps)))/double(_npart);
